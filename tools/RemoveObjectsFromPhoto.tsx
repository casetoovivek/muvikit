...
const RemoveObjectsFromPhoto: React.FC = () => {
    ...
    const handleRemoveObject = async () => {
        ...
        try {
            ...
            const prompt = `From the image provided, please remove the following object: "${objectToRemove}". Fill in the space it occupied realistically and seamlessly to match the surrounding background and textures.`;
            ...
            console.error(err);
            setError('Failed to remove the object. The request may have violated a safety policy. Please try a different image or description.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div className="space-y-6">
            <div className="pb-4 border-b border-gray-200 dark:border-slate-700">
                <h2 className="text-3xl font-bold text-slate-800 dark:text-slate-100">Remove Objects from Photo</h2>
                <p className="mt-1 text-lg text-slate-500 dark:text-slate-400">Erase unwanted objects or people from photos using AI.</p>
            </div>
            ...
            <div className="bg-white p-6 rounded-lg border border-slate-200 space-y-4 dark:bg-slate-800 dark:border-slate-700">
                ...
                />
                 {originalImage && (
                    <div>
                        <label htmlFor="object-description" className="block text-sm font-medium text-gray-700 dark:text-slate-300">Object to Remove</label>
                        <input
                            type="text"
                            id="object-description"
                            value={objectToRemove}
                            onChange={(e) => setObjectToRemove(e.target.value)}
                            placeholder="e.g., 'the person in the red shirt', 'the car on the left'"
                ...
                >
                    {isLoading ? <><SpinnerIcon className="w-5 h-5 mr-2 animate-spin" /> Removing Object...</> : 'Remove Object'}
                </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {processedImage && (
                    <div className="text-center">
                        ...
                        <img src={processedImage} alt="Object removed" className="max-h-96 mx-auto rounded-lg shadow-md" />
                         <a
                            href={processedImage}
